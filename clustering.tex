...
%   7. application in real world dataset
%   8. 总结与展望
\section{Background}
...
\end{proof}
\begin{remark}
Theorem \ref{thm:hierachical} shows that for a series of threshold $\gamma$, $\P_{\gamma}$ forms a \textbf{hierarchical clustering} structure.
\end{remark}
...
Let $\P_{\gamma_0} = \{V\}$, then the hierachical partition structure is $\P_{\gamma_0} \prec \P_{\gamma_1} \prec \dots \P_{\gamma_N}$.
...
\begin{remark}
From the definition of MMI, we can get the scaling-invariant property of info-clustering.
If we use MMI to do clustering, scaling $ \norm{\widetilde{B}_{ij}}_F^2$ does not affect the clustering result.
\end{remark}
\section{Property}
\subsection{Origin}
Our definition \eqref{eq:newDef} comes from an approximation of KL-Divergence under certain conditions. See appendix \ref{sec:akld} for detail.
...
The smallest critical value of $\mathcal{C}_{\gamma}(Z_V)$ is $I(Z_V)$  and for each $B \in \P$, since  our $\P$ is fundamental partition we have $I(Z_B) > I(Z_V)$ by Theorem \ref{thm:strict_larger_mi} for $\abs{B}>1$. Take $\gamma = I(Z_V)$, by lemma \ref{lem:LargeIZV}, if $I(Z_C) > \gamma$ we have $C\subseteq B' \in \P$. Therefore $B$ is maximal and $B\in \mathcal{C}_{\gamma}(Z_V)$. The same argument can be done for subtrees by replacing $V$ by subroot. Therefore, all tree node sets by top-down approach belong to $\mathcal{C}_{\gamma}(Z_V)$. By Theorem 2.4 in \cite{ic}, we conclude all clusters in $C_{\gamma}(Z_V)$ is a tree node set by top-down approach.
...
Suppose there are $N$ normal points and the index set is $V$. For newly entered points with index $j$, we can predict whether it is normal points by comparing $J_T(Z_{V'})$ with $\gamma_N$, where $V' = V\cup \{j\}$. That is
$$
\frac{(N-1)\gamma_N + \sum_{i \in V}w_{ij}}{N} > \gamma_N
$$
Simplifying the above equation we get the boundary condition whether predicted data point $x$ is normal point (Suppose $V=\{1,2,\dots, N \}$ ):
\begin{equation}
...
\begin{figure}[!ht]
	\centering
	\IfFileExists{experiment/outlier_detection/build/outlier_compare.eps}
	{\includegraphics[width=11cm]{experiment/outlier_detection/build/outlier_compare.eps}}
	{} % not up-to-date
...
\bibitem{psp} The Principal Lattice of Partitions of a Submodular Function
\bibitem{agg} Agglomerative Info-Clustering
\end{thebibliography}
\end{document}
