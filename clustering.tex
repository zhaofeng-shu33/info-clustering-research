...
\end{theorem}
\begin{proof}
Consider $C \in \P_1 \vee \P_2$, then $C = \cup_{i=1}^k B_i ,B_i\in \P_1$.
If $k\geq 2$, from lemma \ref{lem:smallZB} we have $I_{\P_C}(Z_C) \leq I(Z_V)$
where $\P_C=\{B_1, \dots, B_k\}$.
Then from lemma \ref{lem:ref_combination}, we have
\begin{equation*}
I_{\P_1}(Z_V) = \theta I_{\P'}(Z_V) + (1-\theta) I_{\P_C}(Z_C)
\end{equation*}
where $\P' = C \cup \{B\in \P_1| B\cap C= \emptyset\}$
\end{proof}
...
& \overbrace{\geq}_{(b)} \frac{1}{k-1}\sum_{i=1}^{k-1} I(Z_{C_i} \wedge Z_{\cup_{j=i+1}^k C_j}) \\
