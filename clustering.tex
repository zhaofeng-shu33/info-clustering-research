...
\end{equation}
We use the notation $\P_{\gamma}$ to denote the partition associated with $C_{\gamma}(Z_V)$.$\P_{\gamma}$ is obtained by adding all the singletons.
...
\item $I(Z_V) \geq I_{\P_B}(Z_B) \geq I(Z_B)$ if $\exists \P_B \subseteq \P^*$.
...
Then we have
\begin{align*}
I(Z_V) & \upbrace{\geq}_{(a)} I_{\P_U}(Z_U)  = I(Z_{C_1} \wedge \dots \wedge Z_{C_k})\\
& = {1\over k-1}\sum_{i=1}^{k=1} I(Z_{C_1} \wedge Z_{})
\end{align*}•
\end{enumerate}
\end{proof}
