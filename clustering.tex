...
If we can find a vector $ x^* \in P(f_{\lambda}) $ and a complete partition $\P^*$ containing only tight partations,
then $\P$ satisifies
\begin{align*}
...
\ENSURE the partation $\P^*$ s.t. $h(\lambda) = h_{\lambda}(\P^*)$ and the value of $h(\lambda)$
...
\begin{table}[!ht]
\centering
\input{code/utility/compare.tex}
