...
\item For \texttt{mf}, we can only use push-relabel maximum flow algorithm\cite{Goldberg}. $(T,g',\tilde{f})=$\texttt{mf}$(\widetilde{G}(\widetilde{V}, \widetilde{E}), c^{\tilde{\lambda}_2}(e), \tilde{f})$. The first parameter of \texttt{mf} is graph topology structure; the second parameter is edge capacity map and the third parameter is a valid flow to be used as the initial flow of the push-relabel algorithm. \texttt{mf} returns three values, $T$ is minimum cut of target side($t\in T$); $g'$ is the maximum flow value; $\tilde{f}$ is the maximum flow map.

For $\texttt{mf}$, we can choose different active node selecting scheme. For example, highest label selection rule is known to be practically fastest.
...
Within \texttt{split}$(T_l, T_r)$ \footnotemark, suppose equation \eqref{eq:solutionProperty} holds with $(\lambda_1, \lambda_3)$, then 
...
\footnotetext{we ignore the third parameter $\tilde{f}$ for notation convenience }
By deduction, we conclude equation \eqref{eq:solutionProperty} holds and line \eqref{findLambda} always has solution.
\end{proof}
\item In line \ref{alg:flowModify} of Algorithm \ref{alg:pmfC}, flow $\tilde{f}$ is modified in place. We can show $\tilde{f}$ is always a valid preflow for $T_l$. For $T_l=T_0$, this claim holds since $\tilde{f}$ is a maximum flow of $T_0$. Suppose it holds for \texttt{split}$(T_l, T_r, \tilde{f})$, 
\end{enumerate}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\STATE \texttt{split}$( T_0, T_1, \tilde{f})$
		\FUNCTION{\texttt{split}$(T_l, T_r, \tilde{f})$}
		...
		\STATE modify $\tilde{f}$ such that it is a valid preflow for $(\widetilde{G}(\widetilde{V}, \widetilde{E}), c^{\tilde{\lambda}_2}(e))$ \label{alg:flowModify}
		...
		\STATE \texttt{split}$(T_l, T', \tilde{f})$
		\STATE \texttt{split}$(T', T_r, \tilde{f}')$
		\ELSE
