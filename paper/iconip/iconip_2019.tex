...
\end{equation}
where $I(Z_B)$ is the shared information of $Z_B$.

For the given threshold $\gamma$, $C_{\gamma} (Z_V)$ represents a collection of non-intersecting subset of $V$. Every two sets from $\bigcup_{\gamma} C_{\gamma}(Z_V)$ are either disjoint or have subset relationship. Therefore, they can be arranged in a tree $\mathcal{T}$ with the property that $A$ is a parent of $B$ if $B\subset A$. 

$I(Z_V)$ is defined by the following equation:
\begin{align}
