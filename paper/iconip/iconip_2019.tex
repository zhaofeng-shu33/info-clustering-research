...
\begin{enumerate}
	\item Worst case scenario: suppose the hierarchical tree has such a structure that its depth is $\abs{V} - 1$. Consider complete graph with $\abs{E} = O(\abs{V}^2)$. Then each \texttt{DT} has complexity $n^4$ where $n = \abs{V}$. Then our algorithm has complexity $O(3^4+4^4 + \dots + n^4) = O(\frac{1}{5} n^5)$. Although the exponent component (still 5) is not improved. We have reduced the time complexity by a constant coefficient 5.
	\item Average case scenario: we suppose the hierarchical tree is evenly distributed. To make it easy to analyze, we use an ideal type and this ideal type can be generalized. Suppose $k << n$ and $k$ is fixed in the divide and conquer algorithm.
	Then we have $T(n) = k T(\frac{n}{k}) + O(n^4)$. By Master Theorem, since the critical value is $1$ and the regularity condition is satisfied, $T(n) = O(n^4)$. 
\end{enumerate}
%
% ---- Bibliography ----
%
